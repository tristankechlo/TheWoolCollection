...
if __name__ == "__main__":
    ...
    for tag in tags:
        ...
        nameSpaceMappings = [
            ...
            ["c", Globals.fabric_dir]
        ]

        for entry in nameSpaceMappings:
            namespace = entry[0]
            folder = entry[1]
    ...
    for color in Globals.colors:
        # prepare data
        ref = {"replace": False, "values": []}
        for tag in tags:
            tmp = tag if tag == "stairs" else tag[:-1]
            key = "wool_collection:" + color + "_wool_" + tmp
            ref["values"].append(key)

        # save for fabric
        path = join(Globals.fabric_dir, "data", "c", "tags", "%", "dyed/", color + ".json")
